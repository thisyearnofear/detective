...
export default function BriefingRoom({
  ...
  const countdownEndTime =
    gameState?.phaseEndTime || gameState?.registrationEnds || 0;
  ...
  const handleRegister = async () => {
    console.log(
      "[BriefingRoom] Join investigation initiated for FID:",
      currentPlayer.fid,
    );
    ...
    const flowResult = await flow.executeRegistration(
      ...
      {
        skipPermissions:
          gameState?.config && !gameState.config.monetizationEnabled,
      },
    );
    ...
    try {
      ...
      console.log(
        "[BriefingRoom] Sending investigation join request with TX:",
        txHash,
      );
      const arbitrumWalletAddress = localStorage.getItem(
      ...
      const data = await requestJson<any>("/api/game/register", {
        method: "POST",
        headers: {
      ...
      setError(
        err.name === "TimeoutError"
          ? "Investigation join timed out. Please try again."
          : message,
      );
    } finally {
      setIsLoading(false);
  ...
  const handleReady = async () => {
    setIsLoading(true);
    try {
      await requestJson<any>("/api/game/ready", {
        method: "POST",
    ...
    <>
      ...
      {!hasMinPlayers ? (
        ...
        <div className="bg-gradient-to-br from-green-900/30 to-blue-900/30 border-2 border-green-500/30 rounded-xl p-4 md:p-6 text-center backdrop-blur-sm animate-pulse">
          <span className="text-xs text-green-300 uppercase tracking-wide font-bold">
            🚀 Starting Soon!
          </span>
