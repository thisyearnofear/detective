...
export const ACCESS_CONFIG = {
  ...
  ARBITRUM_NFT: {
    ...
    contractAddress: (process.env.NEXT_PUBLIC_ARBITRUM_NFT_CONTRACT ||
      "") as Address,
    minimumBalance: parseInt(
      process.env.NEXT_PUBLIC_ARBITRUM_NFT_MIN_BALANCE || "1",
    ),
  },
  ...
  MONAD_TOKEN: {
    ...
    contractAddress: (process.env.NEXT_PUBLIC_MONAD_TOKEN_CONTRACT ||
      "") as Address,
    minimumBalance: BigInt(
      process.env.NEXT_PUBLIC_MONAD_TOKEN_MIN_BALANCE || "1000000000000000000",
...
const monadClient = createPublicClient({
  ...
  transport: http(
    process.env.NEXT_PUBLIC_MONAD_RPC_URL || "https://mainnet.base.org",
  ),
});
...
export async function checkUserAccess(
  ...
  try {
    console.log(
      `[AccessControl] Checking access for ${walletAddress}, FID: ${fid}`,
    );
    ...
    if (
      ACCESS_CONFIG.ARBITRUM_NFT.enabled &&
      ACCESS_CONFIG.ARBITRUM_NFT.contractAddress
    ) {
      try {
        ...
        status.requirements.arbitrumNFT.verified =
          Number(nftBalance) >= ACCESS_CONFIG.ARBITRUM_NFT.minimumBalance;
    ...
    if (
      ACCESS_CONFIG.MONAD_TOKEN.enabled &&
      ACCESS_CONFIG.MONAD_TOKEN.contractAddress
    ) {
      try {
        ...
        status.requirements.monadToken.verified =
          tokenBalance >= ACCESS_CONFIG.MONAD_TOKEN.minimumBalance;
        ...
        if (status.requirements.monadToken.verified) {
          ...
          const formattedBalance =
            Number(tokenBalance) /
            Math.pow(10, ACCESS_CONFIG.MONAD_TOKEN.decimals);
...
async function checkWhitelist(
  ...
  try {
    const { requestJson } = await import("./fetcher");
    const data = await requestJson<{ whitelisted: boolean }>(
      "/api/access/whitelist/check",
      {
        method: "POST",
...
export function getAccessRequirementsText(): string {
  ...
  if (ACCESS_CONFIG.ARBITRUM_NFT.enabled) {
    requirements.push(
      `${ACCESS_CONFIG.ARBITRUM_NFT.minimumBalance} Arbitrum NFT(s)`,
    );
  }

  if (ACCESS_CONFIG.MONAD_TOKEN.enabled) {
    const minTokens = formatTokenAmount(
      ACCESS_CONFIG.MONAD_TOKEN.minimumBalance,
      ACCESS_CONFIG.MONAD_TOKEN.decimals,
    );
